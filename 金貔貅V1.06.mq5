...
#define DEF_MART_EMA_SLOW 21
#endif
#ifndef DEF_MART_USE_H4
#define DEF_MART_USE_H4 true
#endif
...
#define DEF_SMC_BREAKER_LOOKBACK 50
#endif
...
#define DEF_MART_H4_EMA_PERIOD  50
#endif
...
input double           InpMartTrailPct           = DEF_MART_TRAIL_PCT;        // ▶ 浮盈回撤多少%平仓(0=关闭)
...
input bool             InpMartUseH4Filter        = DEF_MART_USE_H4;           // ▶ 用H4大周期过滤方向
...
void OnTick()
  {
   ...
   {
      MqlDateTime dtDD;
      TimeToStruct(TimeCurrent(), dtDD);
      string todayDD = StringFormat("%02d-%02d", dtDD.mon, dtDD.day);
      if(todayDD != g_todayDateStr)
      {
         g_todayDateStr = todayDD;
         g_todayMaxDrawdown = 0.0;
         g_todayMaxDDPct = 0.0;
      }
      if(g_cachedMartPnl < 0.0)
      {
         double dd = -g_cachedMartPnl;
         if(dd > g_todayMaxDrawdown)
            g_todayMaxDrawdown = dd;
         double eq = AccountInfoDouble(ACCOUNT_EQUITY);
         if(eq > 0.0)
         {
            double ddPct = dd / eq * 100.0;
            if(ddPct > g_todayMaxDDPct)
               g_todayMaxDDPct = ddPct;
         }
      }
   }
...
void TryMartAddLayer()
  {
   ...
   double spacingPts = InpMartBaseSpacingPts + g_martMaxLayerSeq * InpMartIncSpacingPts;
...
void ManageMartTrailing()
  {
   ...
   double pnl = g_cachedMartPnl;
   double retraceThreshold = g_martBasketPeakPnL * (1.0 - InpMartTrailPct / 100.0);
...
bool CheckDailyLossLock()
  {
   ...
   double dd = (-deltaPnl) / g_dayStartEquity * 100.0;
   ...
   if(force || key != g_dayKey)
     {
      ...
      if(g_closedPnlDirty || force)
        {
         g_dayRealizedPnl = CalcMartClosedPnlToday();
         g_closedPnlDirty = false;
        }
      g_dayStartModulePnl = g_dayRealizedPnl + g_cachedMartPnl;
   ...
   if(InpHedgeTriggerMode == HEDGE_BY_EQUITY_PCT)
     {
      double eq = AccountInfoDouble(ACCOUNT_EQUITY);
      if(eq <= 0.0) return;
      double lossPct = (-floatingPnl) / eq * 100.0;
   ...
   if(martPnl >= releaseThreshold)
     {
      ...
      Print("Hedge released: mart PnL turned positive (", martPnl, ")");
      g_hedgeActive = false;
     }
  }
      ...
      if(closeH4 > 0.0)
        {
         if(InpMartUseH4Filter)
            g_sigH4Confirmed = ((g_sigMartEmaDir == 1 && closeH4 > emaH4[1]) ||
                                (g_sigMartEmaDir == -1 && closeH4 < emaH4[1]));
         else
            g_sigH4Confirmed = true;  // no filter = always confirmed
        }
     }
...
double GetCurrentDayDrawdownPct()
  {
   if(g_dayStartEquity <= 0.0)
      return 0.0;
   if(!g_dayHasModuleActivity)
...
void CreateStatusPanel()
  {
   ...
   // --- Info Lines (only 3 active: LINE0/LINE1/LINE2) ---
...
void UpdateStatusPanel()
  {
   ...
   double dayDd = GetCurrentDayDrawdownPct();
   double spread = GetCurrentSpreadPoints();
   double floatingPnl = g_cachedMartPnl;

   datetime cnNow = GetChinaNow();
   MqlDateTime cnTime;
   TimeToStruct(cnNow, cnTime);
   string nowText = StringFormat("%02d:%02d", cnTime.hour, cnTime.min);
   ...
   // --- Card 1: Basket PnL ---
   ObjectSetString(0, OBJ_CARD1_T, OBJPROP_TEXT, "篮子浮盈");
   ...
   // --- Info Lines (3 active) ---
   // Line 0: 方向 + 层数 + 距下层 + 权益 + 余额
   string distText;
   if(g_martLayerCount <= 0 || g_martLayerCount >= InpMartMaxLayers)
      distText = "-";
   else if(g_sigMartDistToNext < 0)
     {
      ...
      distText = StringFormat("%d点", g_sigMartDistToNext);
   ObjectSetString(0, OBJ_LINE0, OBJPROP_TEXT,
      StringFormat("方向: %s  层: %d/%d  距下层: %s  权益: %.0f  余额: %.0f",
         dirText, g_martLayerCount, InpMartMaxLayers, distText, eq, bal));
   ObjectSetInteger(0, OBJ_LINE0, OBJPROP_COLOR, C'224,231,255');
   ...
   string h4Text = InpMartUseH4Filter ? (g_sigH4Confirmed ? "已通过" : "未通过") : "已关闭";

   // Line 1: EMA + H4过滤 + 信号 + 点差 + 日回撤
   ObjectSetString(0, OBJ_LINE1, OBJPROP_TEXT,
      StringFormat("EMA: %s  H4过滤: %s  信号: %s  点差: %.0f/%d  日回撤: %.2f%%",
         emaDirText, h4Text, sigText, spread, InpMaxSpreadPoints, dayDd));
   color sigColor = C'140,155,180';
   if(g_sigMartEntryOk) sigColor = C'80,200,120';
   else if(g_sigMartEmaDir != 0) sigColor = C'255,200,60';
   ObjectSetInteger(0, OBJ_LINE1, OBJPROP_COLOR, sigColor);

   // Line 2: 风控 + 硬SL + 快熔 + 北京时间 + 对冲状态
   string riskDaily = g_dailyLocked ? "锁定" : "正常";
   string riskHardSL = g_martHardSLLocked ? "锁定" : "正常";
   string riskFast = g_fastLossLocked ? "锁定" : "正常";
   string line2Text = StringFormat("风控: %s  硬SL: %s  快熔: %s  北京: %s",
      riskDaily, riskHardSL, riskFast, nowText);
   if(g_hedgeActive)
      line2Text += "  对冲:" + IntegerToString(g_hedgeCount) + "单/" + DoubleToString(g_hedgeLots,2) + "手";
   ...
   ObjectSetInteger(0, OBJ_LINE2, OBJPROP_COLOR, riskColor);
   ...
   // Line 4: 止盈止损参数显示
   {
      ...
      if(InpMartTrailPct <= 0.0)
         ...
         trailText = StringFormat("%.0f%%回撤 门槛:%.0f美分(TP×%.0f%%) 峰值:%.1f", InpMartTrailPct, trailMinProfit, InpMartTrailMinProfitPerLayer, g_martBasketPeakPnL);
   ...
   // Line 5: 对冲信息行
   {
      string hedgeText = "";
      if(!InpEnableHedge)
         hedgeText = StringFormat("对冲: 已关闭  [权益%%:%.1f%%  绝对:%.0f美分]", InpHedgeLossPercent, InpHedgeAbsoluteUSD);
      else if(g_hedgeActive)
         hedgeText = StringFormat("对冲: 激活中  单数:%d  手数:%.2f  浮盈:%.1f美分", g_hedgeCount, g_hedgeLots, g_hedgePnl);
      else
        {
         if(InpHedgeTriggerMode == HEDGE_BY_EQUITY_PCT)
            hedgeText = StringFormat("对冲: 待命  模式:权益%%  触发:浮亏超%.1f%%  比例:%.0f%%  [绝对:%.0f美分]", InpHedgeLossPercent, InpHedgeRatio*100, InpHedgeAbsoluteUSD);
         else
            hedgeText = StringFormat("对冲: 待命  模式:绝对金额  触发:浮亏超%.0f美分  比例:%.0f%%  [权益:%.1f%%]", InpHedgeAbsoluteUSD, InpHedgeRatio*100, InpHedgeLossPercent);
        }
   ...
   if(InpEntryMode == ENTRY_EMA_ONLY)
   ...
   else
     {
      ...
      int emaScoreVal = emaEffective ? InpSMCWeightEMA : 0;
      int smcMaxScore = InpSMCWeightImbalance + InpSMCWeightSD + InpSMCWeightOB + InpSMCWeightFVG + InpSMCWeightLV + InpSMCWeightBreaker;
      int totalMaxScore = smcMaxScore + InpSMCWeightEMA;
      // 按方向计算实际得分（与入场逻辑一致）
      int totalBull = (emaEffective && g_sigMartEmaDir == 1 ? InpSMCWeightEMA : 0) + (g_smcDirection == 1 ? g_smcScore : 0);
      int totalBear = (emaEffective && g_sigMartEmaDir == -1 ? InpSMCWeightEMA : 0) + (g_smcDirection == -1 ? g_smcScore : 0);

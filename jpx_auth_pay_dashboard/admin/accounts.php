...
$params = [];
if ($q !== '') {
    $where .= ' AND (account_login LIKE ? OR customer_name LIKE ? OR admin_note LIKE ?)';
    $like = '%' . $q . '%';
...
async function saveRow(btn) {
  ...
  const body = new URLSearchParams({
    id: tr.dataset.id,
    customer_name: tr.querySelector('.edit-customer').value,
    account_login: tr.querySelector('.edit-account').value,
    admin_note: tr.querySelector('.edit-note').value
  });
  const res = await fetch('../api/admin_update_account.php', {method:'POST', body});
  const data = await res.json();
  btn.disabled = false;
  alert(data.ok ? '已保存' : (data.msg || '保存失败'));
}
</script>
</body></html>

      ...
      if(adverseDollar * 100.0 >= (double)InpFastLossDistance)
        {
         ...
         // 锁定价 = 窗口峰价（变动前的水位），等价格"完全回吐反向变动"才解锁
         // 不能用 curBid（已是触发瞬间的低/高点），否则下一 Tick 立即解锁
         g_fastLossLockPrice = g_fastLossPeakPrice;
...
void CreateStatusPanel()
  {
   ...
   ObjectSetString(0, OBJ_HEADER, OBJPROP_TEXT, "金貔貅 v1.14");
...
void UpdateStatusPanel()
  {
   ...
   // Line 5: 对冲信息行
   {
      ...
      if(!InpEnableHedge)
         hedgeText = StringFormat("对冲: 已关闭  比例:%.0f%%(%.2f手)  [权益%%:%.1f%%  绝对:%.0f美分]", InpHedgeRatio*100, targetHedgeLot, InpHedgeLossPercent, InpHedgeAbsoluteUSD);
      else if(g_hedgeActive)
        {
        ...
        {
         if(InpHedgeTriggerMode == HEDGE_BY_EQUITY_PCT)
            hedgeText = StringFormat("对冲: 待命  模式:权益%%  触发:浮亏超%.1f%%  比例:%.0f%%(%.2f手)  [绝对:%.0f美分]", InpHedgeLossPercent, InpHedgeRatio*100, targetHedgeLot, InpHedgeAbsoluteUSD);
         else
            hedgeText = StringFormat("对冲: 待命  模式:绝对金额  触发:浮亏超%.0f美分  比例:%.0f%%(%.2f手)  [权益:%.1f%%]", InpHedgeAbsoluteUSD, InpHedgeRatio*100, targetHedgeLot, InpHedgeLossPercent);
        }
...
//| spacing = base + (layerSeq+1)*inc + ATR/Point*coeff*ratio^1.5    |
...
double GetMartSpacingPts()
  {
   double baseSpacing = g_effBaseSpacing + (g_martMaxLayerSeq + 1) * InpMartIncSpacingPts;
   ...
   return g_effBaseSpacing + (g_martMaxLayerSeq + 1) * InpMartIncSpacingPts

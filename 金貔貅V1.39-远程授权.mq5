...
input double           InpMartSpacingStage1Factor   = 0.50;                    // ▶ 阶梯间距1倍率(1-5层)
input int              InpMartSpacingStage2MaxLayer = 10;                      // ▶ 阶梯间距2结束层
input double           InpMartSpacingStage2Factor   = 0.80;                    // ▶ 阶梯间距2倍率(6-10层)
input int              InpMartSpacingStage3MaxLayer = 15;                      // ▶ 阶梯间距3结束层
input double           InpMartSpacingStage3Factor   = 1.00;                    // ▶ 阶梯间距3倍率(11-15层)
input double           InpMartSpacingStage4Factor   = 1.50;                    // ▶ 阶梯间距4倍率(16层以上)
...
input bool             InpHardSLAllowResume      = false;                     // ▶ 硬止损后允许当天恢复开新篮子
input int              InpHardSLResumeMinutes    = 20;                        // ▶ 硬止损后等待几分钟恢复
...
input group "=== 轮转锁仓篮子 ==="
input bool             InpEnableBasketRotation   = false;                    // ▶ 启用浮亏锁仓后开启新篮子
input double           InpRotateLockLoss         = 500.0;                    // ▶ 当前篮子浮亏多少美分触发锁仓
input double           InpRotateHedgeRatio       = 0.80;                     // ▶ 备用锁仓比例(保留敞口=0时生效)
input double           InpRotateKeepExposureLots = 0.10;                     // ▶ 锁仓后保留敞口手数(0=按比例)
input int              InpRotateMaxBaskets       = 10;                       // ▶ 最多轮转篮子数量(最大10)
input double           InpRotateCloseProfit      = 0.0;                      // ▶ 旧篮子总浮盈达到多少平仓
input double           InpRotateCloseLoss        = 0.0;                      // ▶ 旧篮子总浮亏达到多少强平(美分,0=关闭)
input int              InpRotateMaxSameDirection = 5;                        // ▶ 同方向轮转篮子最多数量(0=不限制)
input double           InpRotateSameDirMinGap    = 1500.0;                   // ▶ 同向篮子均价最小距离(美分,0=不限制)
...
int    g_activeBasketIndex = 0;     // 当前新篮子序号: 0=基础Magic, 1=Magic+step...
int    g_frozenBasketCount = 0;     // 已锁仓旧篮子数量(面板显示)
double g_frozenBasketPnl = 0.0;     // 旧篮子总浮盈(马丁+锁仓)
datetime g_rotateCooldownUntil = 0; // 轮转锁仓后新篮子冷却截止时间
double   g_rotateCooldownAtrRatio = 0.0; // 冷却恢复ATR短/长比值
long     g_fastClosingMagic[ROTATE_MAX_BASKETS];   // 异步批量平仓中的Magic
datetime g_fastClosingUntil[ROTATE_MAX_BASKETS];   // 防重复发送截止时间
...
string OBJ_ROT_B6       = "HYB_ROT_B6";
string OBJ_ROT_B7       = "HYB_ROT_B7";
string OBJ_ROT_B8       = "HYB_ROT_B8";
string OBJ_ROT_B9       = "HYB_ROT_B9";
string OBJ_ROT_B10      = "HYB_ROT_B10";
string OBJ_ROT_CLOSE1   = "HYB_ROT_CLOSE1";
string OBJ_ROT_CLOSE2   = "HYB_ROT_CLOSE2";
string OBJ_ROT_CLOSE3   = "HYB_ROT_CLOSE3";
string OBJ_ROT_CLOSE4   = "HYB_ROT_CLOSE4";
string OBJ_ROT_CLOSE5   = "HYB_ROT_CLOSE5";
string OBJ_ROT_CLOSE6   = "HYB_ROT_CLOSE6";
string OBJ_ROT_CLOSE7   = "HYB_ROT_CLOSE7";
string OBJ_ROT_CLOSE8   = "HYB_ROT_CLOSE8";
string OBJ_ROT_CLOSE9   = "HYB_ROT_CLOSE9";
string OBJ_ROT_CLOSE10  = "HYB_ROT_CLOSE10";
...
int    g_hATR_RotateCoolFast = INVALID_HANDLE;  // 轮转冷却恢复ATR短(默认6)
int    g_hATR_RotateCoolSlow = INVALID_HANDLE;  // 轮转冷却恢复ATR长(默认14)
...
bool   IsRotationCooldownBlocked(const ENUM_POSITION_TYPE side);
bool   IsRotationSameDirectionBlocked(const ENUM_POSITION_TYPE side, const double price);
double GetRotationCooldownATRRatio();
string GetRotationActiveGVName();
bool   LoadRotationActiveIndex();
void   SaveRotationActiveIndex();
void   ClearRotationActiveIndex();
...
bool CheckRemoteAuthorization(const bool showAlert)
{
   ...
   url += sep + "account_login=" + account + "&version=1.39R&product=XAUUSD";
...
int OnInit()
  {
   ...
   if(InpRotateCooldownNeedATR)
     {
      g_hATR_RotateCoolFast = iATR(_Symbol, InpMartEntryTF, InpRotateCooldownATRFastPeriod);
      g_hATR_RotateCoolSlow = iATR(_Symbol, InpMartEntryTF, InpRotateCooldownATRSlowPeriod);
      if(g_hATR_RotateCoolFast == INVALID_HANDLE || g_hATR_RotateCoolSlow == INVALID_HANDLE)
         Print("[轮转冷却] ATR恢复句柄初始化失败，将跳过ATR恢复条件");
      else
         PrintFormat("[轮转冷却] ATR恢复OK: TF=%d, fast=%d, slow=%d, resume=%.2f",
                     InpMartEntryTF, InpRotateCooldownATRFastPeriod, InpRotateCooldownATRSlowPeriod,
                     InpRotateCooldownATRResumeRatio);
     }
   ...
   SyncActiveBasketIndex(); // 休市无tick时也要恢复轮转活动篮子
   int timerSec = 0;
...
void OnDeinit(const int reason)
  {
   ...
   if(g_hATR_RotateCoolFast != INVALID_HANDLE) IndicatorRelease(g_hATR_RotateCoolFast);
   if(g_hATR_RotateCoolSlow != INVALID_HANDLE) IndicatorRelease(g_hATR_RotateCoolSlow);
   ...
   if(id == CHARTEVENT_OBJECT_CLICK)
     {
      ...
      else if(GetRotationBasketCloseButtonIndex(sparam) >= 0) // 单个轮转篮子全平
        {
         int basketIndex = GetRotationBasketCloseButtonIndex(sparam);
         CloseRotationBasketByIndex(basketIndex);
         ObjectSetInteger(0, sparam, OBJPROP_STATE, false);
        }
...
void CloseAllRotationBaskets()
  {
   int maxBaskets = GetRotateMaxBaskets();
   for(int i = 0; i < maxBaskets; ++i)
      CloseBasketByMagic(GetBasketMagicByIndex(i));
   g_martLastCloseTime = TimeCurrent();
   g_hedgeActive = false;
   g_cachedMartPnl = 0.0;
   ClearRotationActiveIndex();
  }

string GetRotationActiveGVName()
  {
   return "JPX_ROT_ACTIVE_" + IntegerToString((long)AccountInfoInteger(ACCOUNT_LOGIN)) + "_" + IntegerToString((int)InpMagicNumber);
  }
...
bool LoadRotationActiveIndex()
  {
   if(!InpEnableBasketRotation)
     {
      g_activeBasketIndex = 0;
      return false;
     }

   string gvName = GetRotationActiveGVName();
   int maxBaskets = GetRotateMaxBaskets();
   if(GlobalVariableCheck(gvName))
     {
      int idx = (int)MathRound(GlobalVariableGet(gvName));
      if(idx >= 0 && idx < maxBaskets)
        {
         g_activeBasketIndex = idx;
         PrintFormat("[轮转恢复] 从全局变量恢复活动篮子 B%d", idx + 1);
         return true;
        }
      PrintFormat("[轮转恢复] 全局变量活动篮子无效: %d, 将自动推断", idx + 1);
     }
   return false;
  }
...
void SaveRotationActiveIndex()
  {
   if(!InpEnableBasketRotation) return;
   GlobalVariableSet(GetRotationActiveGVName(), (double)g_activeBasketIndex);
  }
...
void SyncActiveBasketIndex()
  {
   int maxBaskets = GetRotateMaxBaskets();
   if(g_activeBasketIndex < 0 || g_activeBasketIndex >= maxBaskets)
      g_activeBasketIndex = 0;

   int mc, hc;
   double ml, mp, hl, hp;
   ENUM_MART_DIRECTION d;
   GetBasketStatsByMagic(GetActiveMagicNumber(), mc, ml, mp, hc, hl, hp, d);
   if(mc > 0 && hc == 0)
     {
      g_trade.SetExpertMagicNumber(GetActiveMagicNumber());
      SaveRotationActiveIndex();
      return;
     }

   for(int i = maxBaskets - 1; i >= 0; --i)
     {
      GetBasketStatsByMagic(GetBasketMagicByIndex(i), mc, ml, mp, hc, hl, hp, d);
      if(mc > 0 && hc == 0)
        {
         g_activeBasketIndex = i;
         g_trade.SetExpertMagicNumber(GetActiveMagicNumber());
         SaveRotationActiveIndex();
         PrintFormat("[轮转恢复] 自动推断活动篮子 B%d", i + 1);
         return;
        }
     }
   ...
   for(int i = 0; i < maxBaskets; ++i)
     {
      if(BasketHasAnyPosition(i))
         highestUsed = i;
     }

   for(int i = highestUsed + 1; i < maxBaskets; ++i)
     {
      if(!BasketHasAnyPosition(i))
        {
         g_activeBasketIndex = i;
         g_trade.SetExpertMagicNumber(GetActiveMagicNumber());
         SaveRotationActiveIndex();
         PrintFormat("[轮转恢复] 按最高已占用篮子推断活动篮子 B%d", i + 1);
         return;
        }
     }
      ...
      if(!BasketHasAnyPosition(i))
        {
         ...
         g_trade.SetExpertMagicNumber(GetActiveMagicNumber());
         SaveRotationActiveIndex();
         PrintFormat("[轮转恢复] 按空篮子回退推断活动篮子 B%d", i + 1);
         return;
        }
     }
  }

void ManageFrozenRotationBaskets()
  {
   g_frozenBasketCount = 0;
   g_frozenBasketPnl = 0.0;
   if(!InpEnableBasketRotation)
      return;
   ...
   for(int i = 0; i < maxBaskets; ++i)
     {
      if(i == g_activeBasketIndex) continue;
      int mc, hc;
      double ml, mp, hl, hp;
      ENUM_MART_DIRECTION d;
      long magic = GetBasketMagicByIndex(i);
      GetBasketStatsByMagic(magic, mc, ml, mp, hc, hl, hp, d);
      if(mc <= 0 && hc <= 0) continue;

      double totalPnl = mp + hp;
      g_frozenBasketCount++;
      g_frozenBasketPnl += totalPnl;
      if(totalPnl >= InpRotateCloseProfit)
        {
         PrintFormat("[轮转篮子] 旧篮子B%d Magic=%I64d 总浮盈 %.2f >= %.2f, 自动平仓",
                     i + 1, magic, totalPnl, InpRotateCloseProfit);
         CloseBasketByMagic(magic);
        }
      else if(InpRotateCloseLoss > 0.0 && totalPnl <= -InpRotateCloseLoss)
        {
         PrintFormat("[轮转篮子] 旧篮子B%d Magic=%I64d 总浮亏 %.2f <= -%.2f, 止损强平",
                     i + 1, magic, totalPnl, InpRotateCloseLoss);
         CloseBasketByMagic(magic);
        }
     }
  }

int FindEmptyRotationBasket()
  {
   int maxBaskets = GetRotateMaxBaskets();
   for(int i = 0; i < maxBaskets; ++i)
     {
...
bool CheckRotationBasketLock()
  {
   if(!InpEnableBasketRotation) return false;
   if(GetRotateMaxBaskets() <= 1) return false;
   if(InpRotateLockLoss <= 0.0) return false;
   if(InpRotateKeepExposureLots <= 0.0 && InpRotateHedgeRatio <= 0.0) return false;
   if(g_martLayerCount <= 0 || g_martDirection == MART_DIR_NONE) return false;
   if(g_hedgeActive) return false;
   if(g_cachedMartPnl > -InpRotateLockLoss) return false;

   int nextIndex = FindEmptyRotationBasket();
   bool hasNextBasket = (nextIndex >= 0);

   double hedgeLots = 0.0;
   if(InpRotateKeepExposureLots > 0.0)
      hedgeLots = NormalizeVolume(g_martTotalLots - InpRotateKeepExposureLots);
   else
      hedgeLots = NormalizeVolume(g_martTotalLots * InpRotateHedgeRatio);
   if(hedgeLots <= 0.0) return false;
   if(hedgeLots >= g_martTotalLots)
      hedgeLots = NormalizeVolume(g_martTotalLots - SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_STEP));
   if(hedgeLots <= 0.0) return false;

   ENUM_POSITION_TYPE hedgeSide = (g_martDirection == MART_DIR_BUY) ? POSITION_TYPE_SELL : POSITION_TYPE_BUY;
   int oldIndex = g_activeBasketIndex;
   long oldMagic = GetActiveMagicNumber();
   string cmt = StringFormat("%s_R%d", HEDGE_COMMENT, oldIndex + 1);
   if(!PlaceMarketOrder(hedgeSide, hedgeLots, 0.0, 0.0, cmt))
      return false;

   double remainLots = MathMax(0.0, g_martTotalLots - hedgeLots);
   if(hasNextBasket)
      PrintFormat("[轮转篮子] B%d Magic=%I64d 浮亏 %.2f 触发锁仓 %.2f手, 剩余敞口%.2f手, 切换到B%d Magic=%I64d",
                  oldIndex + 1, oldMagic, g_cachedMartPnl, hedgeLots, remainLots,
                  nextIndex + 1, GetBasketMagicByIndex(nextIndex));
   else
      PrintFormat("[轮转篮子] B%d Magic=%I64d 浮亏 %.2f 触发最终锁仓 %.2f手, 剩余敞口%.2f手, 轮转篮子已满不再切换",
                  oldIndex + 1, oldMagic, g_cachedMartPnl, hedgeLots, remainLots);

   if(!hasNextBasket)
     {
      RefreshHedgeState();
      g_noEntryReason = "轮转篮子已满，最后篮子已锁仓";
      g_rotateCooldownUntil = 0;
      return true;
     }

   g_activeBasketIndex = nextIndex;
   g_trade.SetExpertMagicNumber(GetActiveMagicNumber());
   SaveRotationActiveIndex();
   g_martDirection = MART_DIR_NONE;
   g_martLayerCount = 0;
   ...
   g_hedgePnl = 0.0;
   g_martLastLayerTime = 0;
   if(InpRotateCooldownMinutes > 0)
     {
      g_rotateCooldownUntil = TimeCurrent() + InpRotateCooldownMinutes * 60;
      g_noEntryReason = StringFormat("轮转冷却中(%d分钟)", InpRotateCooldownMinutes);
      PrintFormat("[轮转冷却] 新篮子B%d 冷却至 %s，冷却后ATR/防追按参数复核",
                  nextIndex + 1, TimeToString(g_rotateCooldownUntil, TIME_DATE|TIME_MINUTES));
     }
   else
     {
      g_rotateCooldownUntil = 0;
...
void TryMartEntry()
  {
   ...
   if(InpEnableBasketRotation && g_rotateCooldownUntil > 0 && TimeCurrent() < g_rotateCooldownUntil)
     {
      int remainSec = (int)(g_rotateCooldownUntil - TimeCurrent());
      int remainMin = (remainSec + 59) / 60;
      g_noEntryReason = StringFormat("轮转冷却中(%d分钟)", remainMin);
      return;
     }
   ...
   if(IsRotationCooldownBlocked(side))
      return;

   if(IsRotationSameDirectionBlocked(side, price))
      return;
...
int GetRotateMaxBaskets()
  {
   if(!InpEnableBasketRotation)
      return 1;
   int maxBaskets = InpRotateMaxBaskets;
   if(maxBaskets < 1) maxBaskets = 1;
   if(maxBaskets > ROTATE_MAX_BASKETS) maxBaskets = ROTATE_MAX_BASKETS;
   return maxBaskets;
  }
...
long GetBasketMagicByIndex(const int index)
  {
   int idx = index;
   if(idx < 0) idx = 0;
   int step = InpRotateMagicStep;
   if(step <= 0) step = 1;
   return InpMagicNumber + (long)idx * step;
  }

long GetActiveMagicNumber()
  {
   return GetBasketMagicByIndex(g_activeBasketIndex);
  }
...
bool IsRotationSameDirectionBlocked(const ENUM_POSITION_TYPE side, const double price)
  {
   if(!InpEnableBasketRotation)
      return false;
   if(InpRotateMaxSameDirection <= 0 && InpRotateSameDirMinGap <= 0.0)
      return false;

   ENUM_MART_DIRECTION targetDir = (side == POSITION_TYPE_BUY) ? MART_DIR_BUY : MART_DIR_SELL;
   string dirText = (targetDir == MART_DIR_BUY) ? "多" : "空";
   int sameCount = 0;
   double nearestGap = DBL_MAX;
   int nearestIndex = -1;
   double nearestAvg = 0.0;
   ...
   for(int i = 0; i < maxBaskets; ++i)
     {
      ...
      sameCount++;
      double lots = 0.0;
      double avg = GetBasketMartAvgPriceByMagic(magic, targetDir, lots);
      if(avg > 0.0 && lots > 0.0)
        {
         double gapCents = MathAbs(price - avg) * 100.0;
         if(gapCents < nearestGap)
           {
            nearestGap = gapCents;
            nearestIndex = i;
            nearestAvg = avg;
   ...
   if(InpRotateMaxSameDirection > 0 && sameCount >= InpRotateMaxSameDirection)
     {
      g_noEntryReason = StringFormat("轮转同向限制: %s篮子%d/%d", dirText, sameCount, InpRotateMaxSameDirection);
      return true;
     }

   if(InpRotateSameDirMinGap > 0.0 && nearestIndex >= 0 && nearestGap < InpRotateSameDirMinGap)
     {
      g_noEntryReason = StringFormat("轮转同价限制: %sB%d均价%.2f 距%.0f<%.0f美分",
                                     dirText, nearestIndex + 1, nearestAvg, nearestGap, InpRotateSameDirMinGap);
      return true;
     }
...
bool IsRotationMagic(const long magic)
  {
   int maxBaskets = GetRotateMaxBaskets();
   for(int i = 0; i < maxBaskets; ++i)
     {
      if(magic == GetBasketMagicByIndex(i))
         return true;
     }
   return false;
  }
...
string GetRotationBasketLabelName(const int index)
  {
   ...
   if(index == 4) return OBJ_ROT_B5;
   if(index == 5) return OBJ_ROT_B6;
   if(index == 6) return OBJ_ROT_B7;
   if(index == 7) return OBJ_ROT_B8;
   if(index == 8) return OBJ_ROT_B9;
   return OBJ_ROT_B10;
  }

string GetRotationBasketCloseButtonName(const int index)
  {
   if(index == 0) return OBJ_ROT_CLOSE1;
   if(index == 1) return OBJ_ROT_CLOSE2;
   if(index == 2) return OBJ_ROT_CLOSE3;
   if(index == 3) return OBJ_ROT_CLOSE4;
   if(index == 4) return OBJ_ROT_CLOSE5;
   if(index == 5) return OBJ_ROT_CLOSE6;
   if(index == 6) return OBJ_ROT_CLOSE7;
   if(index == 7) return OBJ_ROT_CLOSE8;
   if(index == 8) return OBJ_ROT_CLOSE9;
   return OBJ_ROT_CLOSE10;
  }

int GetRotationBasketCloseButtonIndex(const string name)
  {
   if(name == OBJ_ROT_CLOSE1) return 0;
   if(name == OBJ_ROT_CLOSE2) return 1;
   if(name == OBJ_ROT_CLOSE3) return 2;
   if(name == OBJ_ROT_CLOSE4) return 3;
   if(name == OBJ_ROT_CLOSE5) return 4;
   if(name == OBJ_ROT_CLOSE6) return 5;
   if(name == OBJ_ROT_CLOSE7) return 6;
   if(name == OBJ_ROT_CLOSE8) return 7;
   if(name == OBJ_ROT_CLOSE9) return 8;
   if(name == OBJ_ROT_CLOSE10) return 9;
   return -1;
  }
...
void PrepareRotationBasketCloseButton(const string name, const int x, const int y)
  {
   if(ObjectFind(0, name) < 0)
   ...
   ObjectSetInteger(0, name, OBJPROP_XSIZE, 24);
   ObjectSetInteger(0, name, OBJPROP_YSIZE, 18);
   ObjectSetString(0, name, OBJPROP_FONT, "Microsoft YaHei UI");
   ObjectSetInteger(0, name, OBJPROP_FONTSIZE, 8);
   ObjectSetString(0, name, OBJPROP_TEXT, "平");
   ObjectSetInteger(0, name, OBJPROP_COLOR, clrWhite);
   ObjectSetInteger(0, name, OBJPROP_BORDER_COLOR, C'150,160,180');
   ObjectSetInteger(0, name, OBJPROP_SELECTABLE, false);
   ObjectSetInteger(0, name, OBJPROP_HIDDEN, true);
  }
   ...
   for(int i = 0; i < ROTATE_MAX_BASKETS; ++i)
     {
      ...
      string btnName = GetRotationBasketCloseButtonName(i);
      if(ObjectFind(0, btnName) >= 0)
        {
         ObjectSetInteger(0, btnName, OBJPROP_YDISTANCE, -9999);
         ObjectSetInteger(0, btnName, OBJPROP_STATE, false);
        }
     }
  }

bool CloseRotationBasketByIndex(const int index)
  {
   if(index < 0 || index >= GetRotateMaxBaskets())
      return false;

   long magic = GetBasketMagicByIndex(index);
   if(!BasketHasAnyPosition(index))
     {
      PrintFormat("[轮转篮子] B%d Magic=%I64d 无持仓可平", index + 1, magic);
      return false;
     }

   bool closed = CloseBasketByMagic(magic);
   if(closed)
     {
      PrintFormat("[轮转篮子] 手动全平 B%d Magic=%I64d", index + 1, magic);
      if(InpShowStatusPanel) UpdateStatusPanel();
     }
   return closed;
  }
...
void UpdateRotationBasketLabels(const int y)
  {
   ...
   ObjectSetString(0, OBJ_ROT_TITLE, OBJPROP_TEXT, "轮转:");
   ObjectSetInteger(0, OBJ_ROT_TITLE, OBJPROP_COLOR, C'180,190,210');

   int maxBaskets = GetRotateMaxBaskets();
   int startX = g_panelX + 50;
   int itemW = 100;
   int itemH = 22;
   int perRow = 5;
   for(int i = 0; i < ROTATE_MAX_BASKETS; ++i)
     {
      string name = GetRotationBasketLabelName(i);
      string btnName = GetRotationBasketCloseButtonName(i);
      int itemX = startX + (i % perRow) * itemW;
      int itemY = y + (i / perRow) * itemH;
      PrepareRotationBasketLabel(name, itemX, itemY);
      PrepareRotationBasketCloseButton(btnName, itemX + 72, itemY);
      if(i >= maxBaskets)
        {
         ObjectSetString(0, name, OBJPROP_TEXT, "");
         ObjectSetInteger(0, name, OBJPROP_YDISTANCE, -9999);
      ...
      bool isClosing = IsMagicFastClosing(GetBasketMagicByIndex(i));
      double totalPnl = mp + hp;
      string activeMark = (i == g_activeBasketIndex) ? "*" : "";
      string dirText = "--";
      if(d == MART_DIR_BUY) dirText = "多";
      else if(d == MART_DIR_SELL) dirText = "空";
      string text = isClosing
         ? StringFormat("%sB%d%s:平中", dirText, i + 1, activeMark)
         : (hasPosition
            ? StringFormat("%sB%d%s:%+.0f", dirText, i + 1, activeMark, totalPnl)
            : StringFormat("%sB%d%s:-", dirText, i + 1, activeMark));
      ...
      ObjectSetInteger(0, name, OBJPROP_YDISTANCE, itemY);
      ObjectSetInteger(0, btnName, OBJPROP_BGCOLOR, (hasPosition || isClosing) ? C'95,105,125' : C'55,62,78');
      ObjectSetInteger(0, btnName, OBJPROP_YDISTANCE, itemY - 2);
      ObjectSetInteger(0, btnName, OBJPROP_STATE, false);
     }
  }
...
double GetRotationCooldownATRRatio()
  {
   g_rotateCooldownAtrRatio = 0.0;
   if(g_hATR_RotateCoolFast == INVALID_HANDLE || g_hATR_RotateCoolSlow == INVALID_HANDLE)
      return 0.0;
   double atrFast = GetATRValue(g_hATR_RotateCoolFast);
   double atrSlow = GetATRValue(g_hATR_RotateCoolSlow);
   if(atrFast <= 0.0 || atrSlow <= 0.0)
      return 0.0;
   g_rotateCooldownAtrRatio = atrFast / atrSlow;
   return g_rotateCooldownAtrRatio;
  }

bool IsRotationCooldownBlocked(const ENUM_POSITION_TYPE side)
  {
   if(!InpEnableBasketRotation || g_rotateCooldownUntil <= 0)
      return false;

   datetime now = TimeCurrent();
   if(now < g_rotateCooldownUntil)
     {
      int remainSec = (int)(g_rotateCooldownUntil - now);
      int remainMin = (remainSec + 59) / 60;
      g_noEntryReason = StringFormat("轮转冷却中(%d分钟)", remainMin);
      return true;
     }

   if(InpRotateCooldownNeedATR && InpRotateCooldownATRResumeRatio > 0.0)
     {
      double ratio = GetRotationCooldownATRRatio();
      if(ratio > 0.0 && ratio > InpRotateCooldownATRResumeRatio)
        {
         g_noEntryReason = StringFormat("轮转等ATR恢复 %.2f>%.2f", ratio, InpRotateCooldownATRResumeRatio);
         return true;
        }
     }

   if(InpRotateCooldownNeedChaseOK && IsEntryChaseBlocked(side))
     {
      if(g_noEntryReason == "")
         g_noEntryReason = "轮转等首单防追通过";
      return true;
     }

   PrintFormat("[轮转冷却] 已解除: ATR=%.2f 阈值=%.2f", g_rotateCooldownAtrRatio, InpRotateCooldownATRResumeRatio);
   g_rotateCooldownUntil = 0;
   g_rotateCooldownAtrRatio = 0.0;
   return false;
  }
...
bool ManageHedgeRelease()
  {
   if(InpEnableBasketRotation) return false; // 轮转旧篮子由 ManageFrozenRotationBaskets 按总浮盈退出
...
void ResolvePanelLayout()
  {
   ...
   int desiredH = MathMax(430, InpPanelHeight);
   g_panelW = desiredW;
   g_panelH = desiredH;
...
void CreateStatusPanel()
  {
   ...
   ObjectSetString(0, OBJ_HEADER, OBJPROP_TEXT, "金貔貅 v1.39R");
...
void DestroyStatusPanel()
  {
   ...
   ObjectDelete(0, OBJ_ROT_B4); ObjectDelete(0, OBJ_ROT_B5); ObjectDelete(0, OBJ_ROT_B6);
   ObjectDelete(0, OBJ_ROT_B7); ObjectDelete(0, OBJ_ROT_B8); ObjectDelete(0, OBJ_ROT_B9); ObjectDelete(0, OBJ_ROT_B10);
   ObjectDelete(0, OBJ_ROT_CLOSE1); ObjectDelete(0, OBJ_ROT_CLOSE2); ObjectDelete(0, OBJ_ROT_CLOSE3);
   ObjectDelete(0, OBJ_ROT_CLOSE4); ObjectDelete(0, OBJ_ROT_CLOSE5); ObjectDelete(0, OBJ_ROT_CLOSE6);
   ObjectDelete(0, OBJ_ROT_CLOSE7); ObjectDelete(0, OBJ_ROT_CLOSE8); ObjectDelete(0, OBJ_ROT_CLOSE9); ObjectDelete(0, OBJ_ROT_CLOSE10);
...
void UpdateStatusPanel()
  {
   ...
   // Line 5: 对冲信息行
   {
      ...
      if(InpEnableBasketRotation)
        {
         string oldLossText = (InpRotateCloseLoss > 0.0) ? StringFormat("  旧亏损>%.0f强平", InpRotateCloseLoss) : "";
         string rotateLockText = (InpRotateKeepExposureLots > 0.0)
            ? StringFormat("当前浮亏>%.0f留%.2f手", InpRotateLockLoss, InpRotateKeepExposureLots)
            : StringFormat("当前浮亏>%.0f锁%.0f%%", InpRotateLockLoss, InpRotateHedgeRatio * 100.0);
         hedgeText = StringFormat("对冲: 轮转锁仓接管  %s  旧浮盈>%.0f平仓%s",
            rotateLockText, InpRotateCloseProfit, oldLossText);
        }
   ...
   else if(InpEnableBasketRotation)
     {
      string lockText = (InpRotateKeepExposureLots > 0.0)
         ? StringFormat("锁仓:%.0f留%.2f手", InpRotateLockLoss, InpRotateKeepExposureLots)
         : StringFormat("锁仓:%.0f@%.0f%%", InpRotateLockLoss, InpRotateHedgeRatio * 100.0);
      string rotateText = StringFormat("轮转: 活动B%d Magic:%I64d  旧篮子:%d/%d  旧浮盈:%.1f  %s",
         g_activeBasketIndex + 1, GetActiveMagicNumber(), g_frozenBasketCount, GetRotateMaxBaskets(),
         g_frozenBasketPnl, lockText);
      ObjectSetString(0, OBJ_LINE3, OBJPROP_TEXT, rotateText);
      ObjectSetInteger(0, OBJ_LINE3, OBJPROP_YDISTANCE, line3Y);
      ObjectSetInteger(0, OBJ_LINE3, OBJPROP_COLOR, g_frozenBasketCount > 0 ? C'255,200,60' : C'140,155,180');
     }
   else
     {
      ObjectSetString(0, OBJ_LINE3, OBJPROP_TEXT, "");

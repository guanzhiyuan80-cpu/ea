...
**命名规范：**
- 远程授权版：`青鸾V{版本号}-远程授权.mq5`（如 `青鸾V1.62-远程授权.mq5`），通过 HTTP API 到后台验证交易账号授权状态，是后续唯一维护主线。
- 本地授权标准版：已停止维护。V1.62 后的策略逻辑、风控逻辑、面板逻辑升级**只更新远程授权版**，不要再创建/编译/提交新的本地授权版。
- 历史本地授权文件和旧 `金貔貅V{版本号}.mq5` 仅作为归档参考，不再同步新功能。
...
4. **编译验证**：只编译远程授权版，确保 0 errors / 0 warnings
...
**修改**
- 顶栏“稳/中/快”档位新增 EMA 入场严格度联动：稳=EMA 30 分，中=EMA 20 分，快=EMA 10 分。
- 保留原档位 ATR 节奏差异：稳 ATR0.20、中 ATR0.15、快 ATR0.10，并继续叠加账号随机偏移。
- 实际开单判断、面板 `EMA多/空 当前/阈值`、条件状态和未建仓诊断统一使用 `GetEntryEmaThreshold()`，避免显示与交易逻辑不一致。

### 2026-07-29（EA V1.61）

**修改**
- 默认马丁加层间距从 `200/30/0.03` 调整为 `700/100/0.03`，目标让 25 层理论覆盖约 60 美金级别，而不是前几美金内过快堆层。
- 仅修改默认输入值；ATR 动态间距、分段倍率和账号随机偏移逻辑保持不变。

### 2026-07-29（EA V1.62）

**修改**
- RSI 精确过滤跟随顶栏“稳/中/快”动态调整：稳=买<=55/卖>=45，中=买<=58/卖>=42，快=买<=60/卖>=40。
- 实际下单过滤、未建仓原因、精确过滤行和条件状态行统一使用动态 RSI 阈值，避免面板与交易逻辑不一致。
- 旧 `InpEntryBuyRsiMax/InpEntrySellRsiMin` 保留为兼容变量，但不再驱动实际阈值。

### 2026-07-29（维护规则）

**决策**
- 用户确认：后续代码更新不再更新本地授权版，只更新 `青鸾V{版本}-远程授权.mq5`。
- 从 V1.63 起，发布流程只创建/编译/提交远程授权版；本地授权版停留在 V1.62 作为历史归档。

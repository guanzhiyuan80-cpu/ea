...
#define DEF_MART_BASE_SPACING 700
#endif
#ifndef DEF_MART_INC_SPACING
#define DEF_MART_INC_SPACING 100
#endif
...
#define DEF_MART_TRAIL_PCT 60.0
#endif
#ifndef DEF_MART_TRAIL_MIN_PROFIT_PER_LAYER
#define DEF_MART_TRAIL_MIN_PROFIT_PER_LAYER 80.0  // 追踪启动门槛=当前TP×此%，如80=浮盈达TP的80%时启动
#endif
...
#define DEF_FAST_LOSS_EXIT_MAX_LOSS 150.0
#endif
#ifndef DEF_FAST_LOSS_EXIT_MIN_LAYERS
#define DEF_FAST_LOSS_EXIT_MIN_LAYERS 5
#endif
#ifndef DEF_FAST_LOSS_EXIT_NEAR_AVG_CENTS
#define DEF_FAST_LOSS_EXIT_NEAR_AVG_CENTS 200.0
#endif
...
double           InpEntryBuyRsiMax         = 55.0;                      // 固定: 做多RSI过热阈值
double           InpEntrySellRsiMin        = 45.0;                      // 固定: 做空RSI过热阈值
...
bool             InpEntryRequireBodyDirection = false;                  // 固定: M5实体方向仅提示，不硬拦截
...
// 速度模式（运行时按钮动态切换 ATR 系数 + EMA入场严格度）
// 0=稳(ATR0.20/EMA30) 1=中(ATR0.15/EMA20) 2=快(ATR0.10/EMA10)
...
int    g_sigH1RiskDir       = 0;       // 1=H1 strong bull, -1=H1 strong bear, 0=range/weak
double g_sigH1RiskAtrMove   = 0.0;     // 最近3根H1净位移/ATR
...
string GetH1PanelText();
int    GetH1StrongTrendDir(double &atrMove);
...
//+------------------------------------------------------------------+
//| 速度模式切换：覆盖 g_effATRCoeff / EMA入场阈值并高亮按钮          |
//| 0=稳(ATR0.20/EMA30) 1=中(ATR0.15/EMA20) 2=快(ATR0.10/EMA10)       |
//+------------------------------------------------------------------+
int GetEntryEmaThreshold()
{
   int thresholds[3] = {30, 20, 10};
   if(g_speedMode < 0 || g_speedMode > 2) return thresholds[1];
   return thresholds[g_speedMode];
}

double GetEntryBuyRsiMax()
{
   double values[3] = {55.0, 58.0, 60.0};
   if(g_speedMode < 0 || g_speedMode > 2) return values[1];
   return values[g_speedMode];
}

double GetEntrySellRsiMin()
{
   double values[3] = {45.0, 42.0, 40.0};
   if(g_speedMode < 0 || g_speedMode > 2) return values[1];
   return values[g_speedMode];
}
...
void SetSpeedMode(int mode)
{
   ...
   PrintFormat("[速度切换] 模式=%s | ATR系数=%.4f(基准%.2f) EMA入场阈值=%d RSI买<=%.0f/卖>=%.0f",
               label, g_effATRCoeff, base, GetEntryEmaThreshold(),
               GetEntryBuyRsiMax(), GetEntrySellRsiMin());
}
...
bool CheckRemoteAuthorization(const bool showAlert)
{
   ...
   url += sep + "account_login=" + account + "&version=1.62R&product=XAUUSD";
...
bool GetMartSignal(bool &longSignal, bool &shortSignal)
  {
   ...
   int emaPassThreshold = GetEntryEmaThreshold();
   emaLong  = (emaScoreLong  > 0 && emaScoreLong  >= emaPassThreshold);
   emaShort = (emaScoreShort > 0 && emaScoreShort >= emaPassThreshold);

   double h1AtrMove = 0.0;
   int h1RiskDir = GetH1StrongTrendDir(h1AtrMove);
   if(emaLong && h1RiskDir == -1)
     {
      emaLong = false;
      g_noEntryReason = StringFormat("H1强空单边，禁止逆势做多(%.1fATR)", h1AtrMove);
     }
   if(emaShort && h1RiskDir == 1)
     {
      emaShort = false;
      g_noEntryReason = StringFormat("H1强多单边，禁止逆势做空(%.1fATR)", h1AtrMove);
     }

   // V1.54 single entry path: EMA score + H1 strong-trend risk + precision filters.
   longSignal = emaLong;
...
bool IsEntryPrecisionBlocked(const ENUM_POSITION_TYPE side)
  {
   ...
   if(InpEntryMaxDistSlowEMA_ATR > 0.0 && g_sigEntryDistSlowAtr > InpEntryMaxDistSlowEMA_ATR)
     {
      g_noEntryReason = StringFormat("防追单: 距EMA %.2fATR>%.2f", g_sigEntryDistSlowAtr, InpEntryMaxDistSlowEMA_ATR);
      return true;
     }
      ...
      if(CopyBuffer(g_hEntryPrecisionRSI, 0, 1, 1, rsi) >= 1)
        {
         ...
         double buyRsiMax = GetEntryBuyRsiMax();
         double sellRsiMin = GetEntrySellRsiMin();
         if(side == POSITION_TYPE_BUY && buyRsiMax > 0.0 && rsi[0] >= buyRsiMax)
           {
            g_noEntryReason = StringFormat("精确过滤: RSI %.1f>=%.1f 不追多", rsi[0], buyRsiMax);
            return true;
           }
         if(side == POSITION_TYPE_SELL && sellRsiMin > 0.0 && rsi[0] <= sellRsiMin)
           {
            g_noEntryReason = StringFormat("精确过滤: RSI %.1f<=%.1f 不追空", rsi[0], sellRsiMin);
            return true;
           }
        }
     }
      ...
      if(recoveredByBounce && InpFastLossExitMaxLoss > 0.0)
        {
         ...
         double distToAvgCents = 999999.0;
         if(avgPrice > 0.0 && avgLots > 0.0)
           {
            if(g_fastLossLockDir == (int)MART_DIR_BUY)
               distToAvgCents = MathMax(0.0, (avgPrice - curBid) * 100.0);
            else if(g_fastLossLockDir == (int)MART_DIR_SELL)
               distToAvgCents = MathMax(0.0, (curBid - avgPrice) * 100.0);
           }

         bool deepEnough = (g_martLayerCount >= DEF_FAST_LOSS_EXIT_MIN_LAYERS);
         bool stillFarFromAvg = (distToAvgCents > DEF_FAST_LOSS_EXIT_NEAR_AVG_CENTS);
         if(deepEnough && stillFarFromAvg && pnl >= -InpFastLossExitMaxLoss)
           {
            PrintFormat("FastLoss exit flat: layers=%d>=%d recovery=%.0f/%.0f cents pnl=%.2f >= -%.2f distAvg=%.0f>%.0f, close all",
                        g_martLayerCount, DEF_FAST_LOSS_EXIT_MIN_LAYERS,
                        recoveryCents, (double)InpFastLossRecoveryDistance, pnl, InpFastLossExitMaxLoss,
                        distToAvgCents, DEF_FAST_LOSS_EXIT_NEAR_AVG_CENTS);
            CloseAllMartPositions();
            g_martDirection = MART_DIR_NONE;
            g_fastLossLocked = false;
            g_fastLossStartTime = 0;
           ...
           {
            PrintFormat("FastLoss exit skipped: layers=%d/%d recovery=%.0f/%.0f pnl=%.2f limit=-%.2f distAvg=%.0f/%.0f",
                        g_martLayerCount, DEF_FAST_LOSS_EXIT_MIN_LAYERS,
                        recoveryCents, (double)InpFastLossRecoveryDistance, pnl, InpFastLossExitMaxLoss,
                        distToAvgCents, DEF_FAST_LOSS_EXIT_NEAR_AVG_CENTS);
           }
...
void ComputeSignalDiagnostics()
  {
      ...
      if(close1 > 0.0)
        {
         ...
         int passTh = GetEntryEmaThreshold();
   ...
   g_sigH1RiskDir = GetH1StrongTrendDir(g_sigH1RiskAtrMove);

   // Legacy H4 diagnostics kept only for old helper compatibility; H1 risk drives the panel now.
   {
   ...
   // V1.54 uses a single EMA/H1-risk/precision entry path; SMC diagnostics are disabled.
  }
...
int GetH1StrongTrendDir(double &atrMove)
  {
   ...
   if(open1 <= 0.0 || close1 <= 0.0 || open2 <= 0.0 || close2 <= 0.0 || open3 <= 0.0 || close3 <= 0.0)
      return 0;

   double netMove = close1 - open3;
   atrMove = MathAbs(netMove) / atr;
   int bullBodies = (close1 > open1 ? 1 : 0) + (close2 > open2 ? 1 : 0) + (close3 > open3 ? 1 : 0);
   int bearBodies = (close1 < open1 ? 1 : 0) + (close2 < open2 ? 1 : 0) + (close3 < open3 ? 1 : 0);
   bool risingCloses = (close1 > close2 && close2 > close3);
   bool fallingCloses = (close1 < close2 && close2 < close3);

   if(netMove > 0.0 && atrMove >= 1.20 && bullBodies >= 2 && risingCloses)
      return 1;
   if(netMove < 0.0 && atrMove >= 1.20 && bearBodies >= 2 && fallingCloses)
      return -1;
   return 0;
  }
      ...
      if(InpUseEntryPrecisionFilter)
        {
         double buyRsiMax = GetEntryBuyRsiMax();
         double sellRsiMin = GetEntrySellRsiMin();
         if(dir == 1 && buyRsiMax > 0.0 && g_sigEntryPrecisionRsi >= buyRsiMax)
           {
            if(reason != "") reason += "；";
            reason += "RSI偏高，不适合做多";
           }
         if(dir == -1 && sellRsiMin > 0.0 && g_sigEntryPrecisionRsi <= sellRsiMin)
           {
            if(reason != "") reason += "；";
            reason += "RSI偏低，不适合做空";
           }
...
string GetH1PanelText()
  {
   if(g_sigH1RiskDir == 0)
      return "震荡";
   if(g_sigH1RiskDir == 1)
      return (g_sigMartEmaDir == -1) ? "强多逆空" : "强多";
   return (g_sigMartEmaDir == 1) ? "强空逆多" : "强空";
  }
...
string GetEntryThresholdLine()
  {
   ...
   string rsiText = StringFormat("RSI%.1f 买<=%.0f 卖>=%.0f",
      g_sigEntryPrecisionRsi, GetEntryBuyRsiMax(), GetEntrySellRsiMin());
   string distText = StringFormat("距EMA%.2f[%.2f-%.2f黄/%.2f红]",
      g_sigEntryDistSlowAtr, InpEntryMinDistSlowEMA_ATR, InpEntryMaxPullbackEMA_ATR, InpEntryMaxDistSlowEMA_ATR);
...
void CreateStatusPanel()
  {
   ...
   ObjectSetString(0, OBJ_HEADER, OBJPROP_TEXT, "青鸾 v1.62R");
...
void UpdateStatusPanel()
  {
   ...
   string trendArrow = "入场:EMA+H1风控+精确过滤";
   ...
   // Line 0: single entry mode summary
   {
      ...
      string h1Short = GetH1PanelText();
      string scoreLine = StringFormat("入场:%s | EMA多:%d/%d 空:%d/%d | H1:%s | 层:%d/%d 距:%s",
         passText, g_sigMartEmaScoreLong, need, g_sigMartEmaScoreShort, need,
         h1Short, g_martLayerCount, InpMartMaxLayers, distText);

      ObjectSetString(0, OBJ_LINE0, OBJPROP_TEXT, scoreLine);
      color sigColor = g_sigMartEntryOk ? C'80,200,120' : C'255,200,60';
   ...
   string reasonFull = GetEntryReasonLine();
   string reasonLine1 = reasonFull;
   string reasonLine2 = "";
   int maxReasonChars = 38;
   if(StringLen(reasonFull) > maxReasonChars)
     {
      int splitPos = -1;
      int searchPos = 0;
      while(true)
        {
         int found = StringFind(reasonFull, "；", searchPos);
         if(found < 0)
            break;
         if(found <= maxReasonChars)
            splitPos = found;
         searchPos = found + 1;
        }
      if(splitPos < 0)
         splitPos = maxReasonChars;
      reasonLine1 = StringSubstr(reasonFull, 0, splitPos + 1);
      reasonLine2 = StringSubstr(reasonFull, splitPos + 1);
     }

   ObjectSetString(0, OBJ_LINE6, OBJPROP_TEXT, reasonLine1);
   ObjectSetInteger(0, OBJ_LINE6, OBJPROP_YDISTANCE, reasonY);
   color reasonColor = C'180,190,210';
   if(StringFind(reasonFull, "锁定") >= 0 || StringFind(reasonFull, "熔断") >= 0)
      reasonColor = C'255,80,80';
   else if(StringFind(reasonFull, "暂停") >= 0 || StringFind(reasonFull, "休市") >= 0)
      reasonColor = C'255,200,60';
   ObjectSetInteger(0, OBJ_LINE6, OBJPROP_COLOR, reasonColor);
   ObjectSetString(0, OBJ_LINE3, OBJPROP_TEXT, reasonLine2);
   ObjectSetInteger(0, OBJ_LINE3, OBJPROP_YDISTANCE, (reasonLine2 == "") ? -9999 : reasonY + 24);
   ObjectSetInteger(0, OBJ_LINE3, OBJPROP_COLOR, reasonColor);

   ObjectSetString(0, OBJ_LINE7, OBJPROP_TEXT, "条件状态:");
   ObjectSetInteger(0, OBJ_LINE7, OBJPROP_YDISTANCE, reasonY + 48);
   ...
   condColor[0] = condOk[0] ? C'40,220,140' : C'255,90,90';
   ...
   bool h1Blocks = ((g_sigMartEmaDir == 1 && g_sigH1RiskDir == -1) || (g_sigMartEmaDir == -1 && g_sigH1RiskDir == 1));
   condOk[1] = !h1Blocks;
   condColor[1] = h1Blocks ? C'255,90,90' : C'40,220,140';
   condTexts[1] = "H1:" + GetH1PanelText();
   if(!InpUseEntryPrecisionFilter)
     {
      condOk[2] = true; condTexts[2] = "RSI:关";
      condOk[3] = true; condTexts[3] = "距EMA:关";
      condOk[4] = true; condTexts[4] = "实体:关";
      condColor[2] = C'40,220,140';
      condColor[3] = C'40,220,140';
      condColor[4] = C'40,220,140';
     }
   else
     {
      double buyRsiMax = GetEntryBuyRsiMax();
      double sellRsiMin = GetEntrySellRsiMin();
      bool buyRsiOk = (g_sigMartEmaDir == 1 && g_sigEntryPrecisionRsi <= buyRsiMax);
      bool sellRsiOk = (g_sigMartEmaDir == -1 && g_sigEntryPrecisionRsi >= sellRsiMin);
      condOk[2] = (buyRsiOk || sellRsiOk);
      condColor[2] = condOk[2] ? C'40,220,140' : C'255,90,90';
      condTexts[2] = (g_sigMartEmaDir == 1)
         ? StringFormat("RSI:%.1f<=%.0f", g_sigEntryPrecisionRsi, buyRsiMax)
         : ((g_sigMartEmaDir == -1) ? StringFormat("RSI:%.1f>=%.0f", g_sigEntryPrecisionRsi, sellRsiMin) : "RSI:等方向");
      bool distTooNear = (InpEntryMinDistSlowEMA_ATR > 0.0 && g_sigEntryDistSlowAtr < InpEntryMinDistSlowEMA_ATR);
      bool distTooFar = (InpEntryMaxDistSlowEMA_ATR > 0.0 && g_sigEntryDistSlowAtr > InpEntryMaxDistSlowEMA_ATR);
      bool distWarn = (!distTooNear && !distTooFar && InpEntryMaxPullbackEMA_ATR > 0.0 && g_sigEntryDistSlowAtr > InpEntryMaxPullbackEMA_ATR);
      condOk[3] = (!distTooNear && !distTooFar);
      condColor[3] = distTooFar || distTooNear ? C'255,90,90' : (distWarn ? C'255,200,60' : C'40,220,140');
      condTexts[3] = distWarn ? StringFormat("距EMA:%.2f偏远", g_sigEntryDistSlowAtr) : StringFormat("距EMA:%.2f", g_sigEntryDistSlowAtr);
      int expectedBodyDir = (g_sigMartEmaDir == 1) ? 1 : ((g_sigMartEmaDir == -1) ? -1 : 0);
      bool bodySizeWarn = (InpEntryMinBody_ATR > 0.0 && g_sigEntryBodyAtr < InpEntryMinBody_ATR);
      bool bodyDirWarn = (expectedBodyDir != 0 && g_sigEntryBodyDir != 0 && g_sigEntryBodyDir != expectedBodyDir);
      string bodyDirText = (g_sigEntryBodyDir == 1) ? "阳" : ((g_sigEntryBodyDir == -1) ? "阴" : "平");
      condOk[4] = true;
      condColor[4] = (bodySizeWarn || bodyDirWarn) ? C'255,200,60' : C'40,220,140';
      condTexts[4] = bodyDirWarn
         ? StringFormat("实体:%s%.2f反向", bodyDirText, g_sigEntryBodyAtr)
         : (bodySizeWarn ? StringFormat("实体:%s%.2f偏小", bodyDirText, g_sigEntryBodyAtr) : StringFormat("实体:%s%.2f", bodyDirText, g_sigEntryBodyAtr));
     }
   condOk[5] = (!InpUseATRAddPause || !g_addAtrPaused);
   condColor[5] = condOk[5] ? C'40,220,140' : C'255,90,90';
   ...
   for(int d = 0; d < 6; ++d)
     {
      ...
      ObjectSetInteger(0, condObjs[c], OBJPROP_YDISTANCE, reasonY + 48 + (d / 3) * 24);
      ObjectSetInteger(0, condObjs[c], OBJPROP_COLOR, condColor[c]);
     }
   ...
   if(InpEntryMode == ENTRY_EMA_ONLY)
   ...
   else
     {
      ...
      // Legacy SMC code unreachable after return; keep compile compatibility.
      bool emaEffective = g_sigMartEntryOk;

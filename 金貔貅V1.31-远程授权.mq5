...
input double           InpRotateHedgeRatio       = 0.80;                     // ▶ 锁仓比例(0.8=锁80%,留20%敞口)
input int              InpRotateMaxBaskets       = 5;                        // ▶ 最多轮转篮子数量(最大5)
...
bool CheckRemoteAuthorization(const bool showAlert)
{
   ...
   url += sep + "account_login=" + account + "&version=1.31R&product=XAUUSD";
...
bool CheckRotationBasketLock()
  {
   ...
   if(InpRotateLockLoss <= 0.0 || InpRotateHedgeRatio <= 0.0) return false;
   ...
   double hedgeLots = NormalizeVolume(g_martTotalLots * InpRotateHedgeRatio);
   ...
   PrintFormat("[轮转篮子] B%d Magic=%I64d 浮亏 %.2f 触发锁仓 %.2f手(%.0f%%), 切换到B%d Magic=%I64d",
               oldIndex + 1, oldMagic, g_cachedMartPnl, hedgeLots, InpRotateHedgeRatio * 100.0,
   ...
   for(int i = 0; i < ROTATE_MAX_BASKETS; ++i)
     {
      string name = GetRotationBasketLabelName(i);
      string btnName = GetRotationBasketCloseButtonName(i);
      int itemX = startX + i * itemW;
      PrepareRotationBasketLabel(name, itemX, y);
      PrepareRotationBasketCloseButton(btnName, itemX + 72, y);
      if(i >= maxBaskets)
        {
         ObjectSetString(0, name, OBJPROP_TEXT, "");
         ObjectSetInteger(0, name, OBJPROP_YDISTANCE, -9999);
      ...
      ObjectSetInteger(0, btnName, OBJPROP_YDISTANCE, y - 2);
      ObjectSetInteger(0, btnName, OBJPROP_STATE, false);
     }
  }
...
void CreateStatusPanel()
  {
   ...
   ObjectSetString(0, OBJ_HEADER, OBJPROP_TEXT, "金貔貅 v1.31R");
...
void UpdateStatusPanel()
  {
      ...
      if(InpEnableBasketRotation)
        {
         ...
         hedgeText = StringFormat("对冲: 轮转锁仓接管  当前浮亏>%.0f锁%.0f%%  旧浮盈>%.0f平仓%s",
            InpRotateLockLoss, InpRotateHedgeRatio * 100.0, InpRotateCloseProfit, oldLossText);
        }
   ...
   if(g_noEntryReason != "")
     ...
     {
      string rotateText = StringFormat("轮转: 活动B%d Magic:%I64d  旧篮子:%d/%d  旧浮盈:%.1f  锁仓:%.0f@%.0f%%",
         g_activeBasketIndex + 1, GetActiveMagicNumber(), g_frozenBasketCount, GetRotateMaxBaskets(),
         g_frozenBasketPnl, InpRotateLockLoss, InpRotateHedgeRatio * 100.0);

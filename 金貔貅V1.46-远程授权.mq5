...
input bool             InpUseEntryPrecisionFilter = true;                     // ▶ 启用首单精确过滤(RSI/回撤/实体)
input ENUM_TIMEFRAMES  InpEntryPrecisionRSITF    = PERIOD_M5;                 // ▶ 精确过滤RSI周期
input int              InpEntryPrecisionRSIPeriod = 14;                       // ▶ 精确过滤RSI参数
input double           InpEntryBuyRsiMax         = 52.0;                      // ▶ 做多RSI需低于(避免追高)
input double           InpEntrySellRsiMin        = 48.0;                      // ▶ 做空RSI需高于(避免追低)
input double           InpEntryMinDistSlowEMA_ATR = 0.20;                    // ▶ 距慢EMA低于N倍ATR不进(动能不足)
input double           InpEntryMaxPullbackEMA_ATR = 1.80;                    // ▶ 距慢EMA高于N倍ATR不进(位置过远)
input double           InpEntryMinBody_ATR       = 0.30;                      // ▶ 上根实体至少N倍ATR
input bool             InpEntryRequireBodyDirection = true;                   // ▶ 上根K实体方向必须同向
...
bool CheckRemoteAuthorization(const bool showAlert)
{
   ...
   url += sep + "account_login=" + account + "&version=1.46R&product=XAUUSD";
...
string GetEntryReasonLine()
  {
   ...
   string h4 = (InpMartH4FilterMode == H4_FILTER_OFF) ? "关" : (g_sigH4Confirmed ? "顺" : "逆");
...
void CreateStatusPanel()
  {
   ...
   ObjectSetString(0, OBJ_HEADER, OBJPROP_TEXT, "金貔貅 v1.46R");
   ...
   for(int j = 0; j < 2; ++j)
     {
      ...
      ObjectSetInteger(0, extraLineObjs[j], OBJPROP_FONTSIZE, 9);
...
void UpdateStatusPanel()
  {
   ...
   // Sub-header with active entry path. SMC is no longer part of the V1.46 entry display.
   ...
   // Line 0: single entry mode summary
   {
      ...
      string h4Short = (InpMartH4FilterMode == H4_FILTER_OFF) ? "关" : (g_sigH4Confirmed ? "顺" : "逆");
   ...
   ObjectSetString(0, OBJ_LINE7, OBJPROP_TEXT, GetEntryThresholdLine());
   ObjectSetInteger(0, OBJ_LINE7, OBJPROP_YDISTANCE, reasonY + 20);
   ObjectSetInteger(0, OBJ_LINE7, OBJPROP_COLOR, C'140,155,180');

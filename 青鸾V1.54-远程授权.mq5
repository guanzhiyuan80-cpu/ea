...
bool CheckRemoteAuthorization(const bool showAlert)
{
   ...
   url += sep + "account_login=" + account + "&version=1.54R&product=XAUUSD";
...
void CreateStatusPanel()
  {
   ...
   ObjectSetString(0, OBJ_HEADER, OBJPROP_TEXT, "青鸾 v1.54R");
...
void UpdateStatusPanel()
  {
   ...
   if(!InpUseEntryPrecisionFilter)
   ...
   else
     {
      ...
      condOk[4] = (bodySizeOk && bodyDirOk);
      condColor[4] = condOk[4] ? C'40,220,140' : C'255,90,90';
      condTexts[4] = StringFormat("实体:%s%.2f", bodyDirText, g_sigEntryBodyAtr);
     }

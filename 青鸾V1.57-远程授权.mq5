...
bool CheckRemoteAuthorization(const bool showAlert)
{
   ...
   url += sep + "account_login=" + account + "&version=1.57R&product=XAUUSD";
...
void CreateStatusPanel()
  {
   ...
   ObjectSetString(0, OBJ_HEADER, OBJPROP_TEXT, "青鸾 v1.57R");
...
void UpdateStatusPanel()
  {
   ...
   if(!InpUseEntryPrecisionFilter)
   ...
   else
     {
      ...
      bool bodyDirOk = (!InpEntryRequireBodyDirection || expectedBodyDir == 0 || g_sigEntryBodyDir == 0 || g_sigEntryBodyDir == expectedBodyDir);
      string bodyDirText = (g_sigEntryBodyDir == 1) ? "阳" : ((g_sigEntryBodyDir == -1) ? "阴" : "平");
      condOk[4] = bodyDirOk;
      condColor[4] = bodyDirOk ? (bodySizeWarn ? C'255,200,60' : C'40,220,140') : C'255,90,90';
      condTexts[4] = bodySizeWarn ? StringFormat("实体:%s%.2f偏小", bodyDirText, g_sigEntryBodyAtr) : StringFormat("实体:%s%.2f", bodyDirText, g_sigEntryBodyAtr);
     }
